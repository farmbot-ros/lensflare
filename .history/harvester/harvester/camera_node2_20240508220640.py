...
class CameraNode(Node):
    ...
    def get_cam_info(self, msg):
        for camera in msg.cameras:
                ...
                self.device_info["model"] = camera.model
                self.device_info["vendor"] = camera.vendor
                self.device_info["serial"] = camera.serial
                self.device_info["ip"] = camera.ip
                self.device_info["subnetmask"] = camera.subnetmask
                self.device_info["defaultgateway"] = camera.defaultgateway
                self.device_info["mac"] = camera.mac
                self.device_info["name"] = camera.name
                self.device_info["version"] = camera.version
                self.device_info["dhcp"] = camera.dhcp
                self.device_info["presistentip"] = camera.presistentip
                self.device_info["lla"] = camera.lla
                self.get_logger().info(f"Camera {self.name} parameters aquired.")
                self.destroy_subscription(self.camera_info_sub)
                return
        self.get_logger().info(f"Could not find camera {self.name}, node will be destroyed.")
        self.destroy_node()
    ...
    def initialize_camera(self):
        while self.is_initialized is None or self.cam_info is None:
            self.get_logger().info(f"Camera {self.name} not initialized yet.")
            return
        camdict = self.device_info
        devices = system.create_device(self.device_info)
